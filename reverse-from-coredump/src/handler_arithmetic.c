...
void mul_handler(re_list_t * instnode){
	...
	re_list_t re_deflist, re_uselist, re_instlist;  	
	re_list_t *def, *usedst, *usesrc, *useimm;
	valset_u tempval; 
...
void mul_resolver(re_list_t* inst, re_list_t *re_deflist, re_list_t *re_uselist){
	...
	switch (instruction->explicit_count) {
		case 2:
		case 3:
			assert(CAST2_USE(src[0]->node)->operand->datatype == CAST2_USE(src[1]->node)->operand->datatype);
			assert(CAST2_USE(src[0]->node)->operand->datatype == CAST2_DEF(dst[0]->node)->operand->datatype);
...
void div_resolver(re_list_t* inst, re_list_t *re_deflist, re_list_t *re_uselist){
	...
	if((!CAST2_USE(src[0]->node)->val_known || !CAST2_USE(src[1]->node)->val_known)
		...
		switch (CAST2_DEF(dst[0]->node)->operand->datatype) {
			case op_byte:
				vt1.byte = (vt3.byte * vtdiv.byte + vt4.byte) % (2 << BYTE_SIZE);
				vt2.byte = (vt3.byte * vtdiv.byte + vt4.byte) / (2 << BYTE_SIZE);
				break;
			case op_word:
				vt1.word = (vt3.word * vtdiv.word + vt4.word) % (2 << WORD_SIZE);
				vt2.word = (vt3.word * vtdiv.word + vt4.word) / (2 << WORD_SIZE);
				break;
			case op_dword:
				vt1.dword = (((long long)vt3.dword) * vtdiv.dword + vt4.dword) % (((long long)2) << DWORD_SIZE);
				vt2.dword = (((long long)vt3.dword) * vtdiv.dword + vt4.dword) / (((long long)2) << DWORD_SIZE);
				break;
			default:
				break;
		}

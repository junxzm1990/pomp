...
//	printf("opsize for destination %d \n", op_size); 
	...
	*raw_insn = &(table_desc->table[op]);
	...
	if ( subtable && ia32_tables[subtable].type != tbl_suffix &&
		...
		if ( prefix && ( sub_size == INVALID_INSN  ||
			...
			//printf("Invalid instruction found later\n");
	...
	if ( table_desc->type == tbl_ext_ext ) {
		...
		//printf("The consumed size is reduced by 1\n");
...
//	printf("return from one lookup %d\n", size);
...
//	printf("WARNING: DEALING WITH ONE INSTRUCTION ================== \n");
...
//DEBUG
//		printf("well, the instruction cannot be looked up\n");
//DEBUG
